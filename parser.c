...
int  aiParsVersion[] = {160, 209, 210, 304, 305, 306, 307, 308, -1};
...
{
  sprintf (szErr, "Parser out of memory: Segment %s Nr. %d", SegName, SegNr);

  macErrorMessage (PARSE_MALLOC, CRITICAL, szErr);
}
...
BOOL fofCheckMand (int iSeg, int iPlus, int iColon)
{
  ...
  if (fMand && (*pPtr == '\0')) {
    ...
    macErrorMessage (PARSE_MANDMISSING, WARNING, szErr);
    return (FALSE);
  }
  return (TRUE);
}
          ...
          if (iLastSeg != SegNr) {
            ...
            if ((*pTmp != '\r') && (*pTmp != '\n')) {
              *pCurPtr = '\0';
              ...
              fovdPrintLog (LOG_TIMM, szErr);
              /*
              macErrorMessage (PARSE_ELEMTOOLONG, WARNING, szErr);
              */
              iLastSeg = SegNr;
            }
          }
          ...
          if (lTok == astToken[i].lToken) 
            {
              ...
              SegNr++;
              /*
                printf ("found: %c%c%c in %d for %d: ", pStart[0], pStart[1], pStart[2], SegNr, i); 
                for (j = 0; j < 100; j++)
                {
                if (pStart[j] != '\n')
                {
      ...
      if (i == (sizeof (astToken) / sizeof (struct stTOKENINFO))) 
        {
          ...
          macErrorMessage (PARSE_ILLEGALELEM, CRITICAL, szErr);
          return (0);
        } 
      else 
        {
...
void free_timm (struct s_TimmInter *p_inter)
{
  ...
  fovdPrintLog (LOG_DEBUG, "Free interchange\n");
...
void free_g22 (struct s_group_22 *a_g22)
{
  void *p_free;

  for (p_g22 = a_g22; p_g22;) 
    {
      free (p_g22->lin);
      for (p_pia = p_g22->pia; p_pia;) 
        {
...
{
  fovdPrintLog (LOG_DEBUG, "Starting Parser\n");
  ...
  fovdPrintLog (LOG_DEBUG, "Returning from Parser\n");
